...
const ConfigTransforms = require('./ConfigTransforms');
const isObject = (val) => val && typeof val === 'object';

const defaultConfigTransforms = {
  babel: new ConfigTransforms({
    file: {
      js: ['babel.config.js'],
    },
  }),
  postcss: new ConfigTransforms({
    file: {
  ...
  eslintConfig: new ConfigTransforms({
    file: {
      js: ['.eslintrc.js'],
  ...
  jest: new ConfigTransforms({
    file: {
      js: ['jest.config.js'],
    },
  }),
  browserslist: new ConfigTransforms({
    file: {
      lines: ['.browerslistrc'],
    },
  }),
...
class Generator {
  ...
  async generate() {
    ...
    // 将package.json中的字段排序
    this.sortPkg();
    this.files['pacage.json'] = JSON.stringify(this.pkg, null, 2) + '\n';
